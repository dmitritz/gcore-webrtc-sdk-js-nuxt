...
<template>
  <div class="flex gap-1 mb-1 flex-wrap">
    <label>
      <input type="checkbox" @change="$emit('toggle', !checked)" :disabled="disabled || devicesList.length === 0"
        :checked="checked && devicesList.length > 0" :id="`${id}-on`" />
      {{ label.substring(0, 1).toUpperCase() + label.substring(1) }}
    </label>
    <select @change="(e) => $emit('change', (e.target as HTMLSelectElement).value)"
      :disabled="disabled || !checked || readonly || devicesList.length === 0" :id="`${id}-select`">
